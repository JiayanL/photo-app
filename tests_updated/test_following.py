...
class TestFollowingListEndpoint(unittest.TestCase):
    ...
    def test_following_get_check_data_structure(self):
        response = utils.issue_get_request('{0}/api/following'.format(root_url), user_id=self.current_user.get('id'))
    ...
    def test_following_get_jwt_required(self):
    ...
    def test_following_post_valid_request_201(self):
        ...
        response = utils.issue_post_request(root_url + '/api/following', json=body, user_id=self.current_user.get('id'))
        # print(response.text)
    ...
    def test_following_post_jwt_required(self):
        user = utils.get_unfollowed_user(self.current_user.get('id'))
        body = {
            'user_id': user.get('id')
        }
    ...
    def test_following_post_missing_user_id_400(self):
        response = utils.issue_post_request(root_url + '/api/following', json={}, user_id=self.current_user.get('id'))
        # print(response.text)
        self.assertEqual(response.status_code, 400)
...
class TestFollowingDetailEndpoint(unittest.TestCase):
    ...
    def test_following_delete_jwt_required(self):
        following_to_delete = utils.get_following_by_user(self.current_user.get('id'))
        following_id = following_to_delete.get('id')
        url = '{0}/api/following/{1}'.format(root_url, following_id)
...
if __name__ == '__main__':
    ...
    suite.addTests([
        ...
        TestFollowingListEndpoint('test_following_get_jwt_required'),   
        ...
        TestFollowingListEndpoint('test_following_post_jwt_required'),   
